...
- **Lot 16a/16b (réseau MTAP glouton + Steiner) en cycle dev** : `reseau_desserte(pre, cout,
  parcelles, desserte_existante, heuristique, mode, skidding_m, ...)` (`R/desserte_reseau.R`) rend
  un objet `foretaccess_reseau` (`sf` LINESTRING des routes créées + `SpatRaster` du réseau + coût
  total). Portage du MTAP→STAP glouton de ForestRoadNetwork (Klemet) : chaque parcelle est
  raccordée au **réseau courant** par le solveur Lot 15 (variante **multi-cible** : A* qui s'arrête
  sur toute cellule de réseau), la route créée grossit le réseau (réutilisation → arborescence).
  Pré-élagage par distance de débardage. Trois heuristiques d'ordre (plus proche, plus gros volume,
  aléatoire reproductible). Nouveau Rust : `heuristic::dist_to_end_multi`, `solver::solve_network` +
  `build_network`, binding `desserte_reseau`.
  **16b (`mode = "steiner"`, Chung & Sessions)** : graphe complet des terminaux (réseau + un nœud
  ...
  vérifié. 24 tests cargo + 16 tests R. Reste **16c** (raccordement/connexité, sortie affinée).
