...
- **Lot en cours** : **Lot 4 — noyau câble (Rust)**. Spec
  `specs/004-cable.md` validée (2026-07-12). **4a** (caténaire +
  Newton), **4b** (faisabilité de travée) et **4c** (primitives
  d’optimisation : `find_lomin`, `test_span`) livrés : 10 bindings
  extendr, 16 tests cargo + 35 tests R. Le placement multi-supports
  `OptPyl_Up` est différé (oracle réel requis). Suivant : 4d (balayage
  360°/pixel + orchestration R + tuilage).
