...
**Lot 10 — Acquisition depuis AOI** ✅ `v0.11.0` (numéroté 10 pour ne pas renuméroter les lots
existants, mais **logiquement en amont du Lot 1**) : téléchargement automatique des entrées
(MNT RGE ALTI 5 m, desserte BD TOPO, forêt BD Forêt v2, obstacles OSM, cadastre) à partir d'une
`AOI.gpkg`, via `happign`/`osmdata` (patron nemeton, config JSON par pays). `acquire_inputs()`
+ résolveur config-driven ; sorties directement consommables par `preprocess()`. Phase 2
(MNH LiDAR HD + volume, BD Forêt v3) reste **hors périmètre**.
